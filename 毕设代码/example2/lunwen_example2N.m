...
NN= [11 12 58 78 98];
for j=1:5
ni=NN(j); nb=2; nc=ni+nb; nt=20;%N=60， 边界2， 内部58
tau=0.01;c=1;
u0= @(x) sin(pi*x);
u0_1= @(x) pi.*cos(pi*x);
u0_2= @(x) -(pi^2).*sin(pi*x);
mu0= @(x) sin(pi*x);
mu0_1= @(x) pi.*cos(pi*x);
mu0_2= @(x) -pi^2.*sin(pi*x);
ureal= @(x,t) exp(t).*sin(pi*x);
mureal= @(x,t) exp(t).*sin(pi*x);
F_1= @(x,t) (exp(2*t).*((sin(pi*x).^2))./(exp(t).*sin(pi*x)+1))+(pi^2+1).*exp(t).*sin(pi*x);
F_2= @(x,t) (exp(2*t).*((sin(pi*x).^2))./(exp(t).*sin(pi*x)+1))+(pi^2+1).*exp(t).*sin(pi*x);
phi = @(r,c) (c.^2.+r.^2).^(1/2); 
phi_r = @(r,c) ((c.*c+r.*r).^(-1/2));%部分
%phi_rr= @(r,c) c^2.*((c.*c+r.*r).^(-3/2));
phi_rr=@(r,c) ((c.*c+r.*r).^(-1/2))-r.*r.*((c.*c+r.*r).^(-3/2));
...
A1=2*phi(DM1,c)-tau*phi_rr(DM1,c);       A2=tau*(uni(:,1)./(uni(:,1)+1)).*phi(DM1,c);
A3=zeros(ni,nc);                         A4=(2+tau*(uni(:,1)./(uni(:,1)+1))).*phi(DM1,c)-tau*phi_rr(DM1,c);
B1v=2*u0(xi)+tau*u0_2(xi);               B2v=-tau*(uni(:,1)./(uni(:,1)+1)).*mu0(xi);
B3v=zeros(ni,1);                         B4v=(-tau*(uni(:,1)./(uni(:,1)+1))+2).*mu0(xi)+tau*mu0_2(xi);
t_0=0;
t_1=tau;
...
A1=2*phi(DM1,c)-tau*phi_rr(DM1,c);       A2=tau*(uni(:,i)./(uni(:,i)+1)).*phi(DM1,c);
A3=zeros(ni,nc);                         A4=(2+tau*(uni(:,i)./(uni(:,i)+1))).*phi(DM1,c)-tau*phi_rr(DM1,c);
B1=2*phi(DM1,c)+tau*phi_rr(DM1,c);       B2=-tau*(uni(:,i)./(uni(:,i)+1)).*phi(DM1,c);
B3=zeros(ni,nc);                         B4=(-tau*(uni(:,i)./(uni(:,i)+1))+2).*phi(DM1,c)+tau*phi_rr(DM1,c);
